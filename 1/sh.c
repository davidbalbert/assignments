...
runcmd(struct cmd *cmd)
{
  ...
  case '<':
    ...
    if (rcmd->mode & O_CREAT)
      fd = open(rcmd->file, rcmd->mode, S_IRUSR | S_IWUSR);
    else
  ...
  case '|':
    ...
    if (fork1() == 0) {
      ...
      if (dup(p[1]) == -1) {
        fprintf(stderr, "dup(p[1]) failed\n");
        exit(-1);
      }

      if (close(p[0]) == -1) {
        fprintf(stderr, "close(p[0]) failed\n");
        exit(-1);
      }
      if (close(p[1]) == -1) {
        fprintf(stderr, "close(p[1]) failed\n");
        exit(-1);
      }
